...
const AdminTransactions: React.FC = () => {
  ...
  return (
    <div className="space-y-6 gold-gradient-bg min-h-screen py-8 px-2 sm:px-6" style={{ fontFamily: 'var(--font-sans)' }}>
      <h1 className="text-4xl font-serif mb-8 gold-text drop-shadow-lg">All Transactions</h1>
      <div className="glassy-card overflow-hidden">
            ...
            <tbody>
              {transactions.map((transaction) => (
                <tr key={transaction.id} className="border-b border-gold hover:bg-gold/10">
                  <td className="px-6 py-4 font-sans gold-text">{transaction.user}</td>
                  <td className="px-6 py-4"><span className={`px-3 py-1 rounded-full text-sm font-sans font-semibold ${transaction.transaction_type === 'BUY' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>{transaction.transaction_type}</span></td>
        ...
        {transactions.length === 0 && (
          <div className="text-center py-12 text-gray-400"><p className="text-lg font-sans">No transactions found</p></div>
        )}
      </div>
    </div>
  );
